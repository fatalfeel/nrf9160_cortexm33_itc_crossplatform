...
//LOG_MODULE_REGISTER(watchdog, CONFIG_ASSET_TRACKER_LOG_LEVEL); //build error when CONFIG_LOG=y
LOG_MODULE_REGISTER(watchdog, CONFIG_LOG_DEFAULT_LEVEL);

#define WATCHDOG_TIMEOUT_MSEC     30000 //30sec
#define WDT_FEED_WORKER_DELAY_MS  WATCHDOG_TIMEOUT_MSEC/2
...
static int watchdog_timeout_install(struct wdt_data_storage *data)
{
	...
	LOG_INF("Watchdog timeout installed. Timeout: %d", WATCHDOG_TIMEOUT_MSEC);
	return 0;
}
